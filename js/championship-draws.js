(function () {
    ...
    function renderCreateForm() {
        ...
        wrap.innerHTML = `
            <div class="card" style="margin:0;">
                ...
                <div class="row" style="margin-top:12px; align-items:end; gap:12px; flex-wrap:wrap;">
                    ...
                    <div class="row" style="gap:8px;">
                        <button id="btnSaveChampionship" type="button">
                            Salvar
                        </button>
    ...
    function renderDrawsList(draws) {
        ...
        wrap.innerHTML = `
            <div style="display:flex; justify-content:flex-end; margin-bottom:12px;">
                <button id="btnNewChampionship" type="button">
                    Novo campeonato
                </button>
            </div>
                            ...
                            >
                                <div>
                                    ...
                                    <div class="muted" style="margin-top:6px;">
                                        Tipo: ${getDrawTypeLabel(draw.draw_type)}
                                    </div>
    ...
    document.addEventListener("click", async (event) => {
        const newButton = event.target.closest("#btnNewChampionship");
        ...
        const cancelButton = event.target.closest("#btnCancelChampionship");
        ...
        const saveButton = event.target.closest("#btnSaveChampionship");

        if (saveButton) {
            const name = ($("championshipName")?.value || "").trim();
            const drawType = $("championshipDrawType")?.value || "simple";

            if (!name) {
                return alert("Informe o nome do campeonato.");
            }
            ...
            try {
                await createChampionshipDraw(name, drawType);
        ...
        const deleteButton = event.target.closest(".btnDeleteChampionship");

        if (deleteButton) {
            const id = deleteButton.dataset.id;
            const name = deleteButton.dataset.name || "este campeonato";
        ...
        const openButton = event.target.closest(".btnOpenChampionship");

        if (openButton) {
            const id = openButton.dataset.id;

            alert(`Abrir campeonato ${id}\n\nEssa será a próxima etapa.`);
        }
    });

    window.renderChampionshipDrawsTab = renderChampionshipDrawsTab;
})();